...
単に地図を表示・塗り分けたいだけなら、県名も入った `dist/japan.svg` をそのまま使う方が簡単です。

![css-grid.html の実行画面](css-grid.png)

## svg-choropleth.html

`data/japan.grid.json` から値を作り、`dist/japan-nolabel.svg` を展開して `id="JP-13"` のような識別子で塗り分けます（コロプレス図）。「値の算出はJSON、描画はSVG」という想定どおりの組み合わせの例です。
